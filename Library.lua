--[[
    ...
    function Library:Watermark(Format: string)
        -- Format supports dynamic tokens that auto-update every second:
        --   {name}     - player DisplayName
        --   {username} - @username
        --   {game}     - game name (via MarketplaceService)
        --   {gameid}   - game PlaceId
        --   {ping}     - network ping in ms
        --   {fps}      - current FPS
        --   {time}     - HH:MM:SS
        --   {date}     - DD/MM/YYYY
        -- Example: Library:Watermark("{game} | {name} | Ping: {ping}ms | {fps} FPS | {time}")
        --
        -- Methods:
        --   Watermark:SetFormat(fmt: string)          -- change format at runtime
        --   Watermark:SetText(text: string)           -- set static text (no tokens)
        --   Watermark:SetUpdateInterval(n: number)    -- change refresh rate (default 1s)
        --   Watermark:SetVisibility(bool: boolean)    -- show/hide
...
local Library do
    ...
    Library.Connect = function(self, Event, Callback, Name)
        ...
        Name = Name or ("C_" .. self.UnnamedConnections)
    ...
    Library.NextFlag = function(self)
        ...
        return "Flag_" .. self.UnnamedFlags
    end
    ...
    Library.Watermark = function(self, Name)
        ...
        -- Format string for dynamic watermark text.
        -- Supported tokens:
        ...
        -- Example: Library:Watermark("{game} | {name} | {ping}ms | {fps} fps | {time}")
        Watermark.Format = Name
        Watermark.UpdateInterval = 1 -- seconds between text refresh
        ...
        -- Resolves all dynamic tokens in the format string
        local function ResolveWatermarkText(fmt)
            ...
            -- Ping
            local ping = 0
            local statsOk, statsResult = pcall(function()
                return game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValue()
            end)
            if statsOk then ping = MathFloor(statsResult) end

            -- FPS (averaged over last frame)
            local fps = MathFloor(1 / RunService.Heartbeat:Wait())

            -- Time
            local t = os.date("*t")
            local timeStr = StringFormat("%02d:%02d:%02d", t.hour, t.min, t.sec)
            local dateStr = StringFormat("%02d/%02d/%04d", t.day, t.month, t.year)

            local displayName = player and player.DisplayName or "Player"
            local userName = player and ("@" .. player.Name) or "@Player"

            local resolved = fmt
            resolved = StringGSub(resolved, "{name}",     displayName)
            resolved = StringGSub(resolved, "{username}", userName)
            resolved = StringGSub(resolved, "{game}",     gameName)
            resolved = StringGSub(resolved, "{gameid}",   placeId)
            resolved = StringGSub(resolved, "{ping}",     tostring(ping))
            resolved = StringGSub(resolved, "{fps}",      tostring(fps))
            resolved = StringGSub(resolved, "{time}",     timeStr)
            resolved = StringGSub(resolved, "{date}",     dateStr)
        ...
        local function StartUpdateLoop()
            ...
            UpdateThread = task.spawn(function()
                while Items["Watermark"] and Items["Watermark"].Instance and Items["Watermark"].Instance.Parent do
                    local ok, text = pcall(ResolveWatermarkText, Watermark.Format)
                    if ok and Items["Title"] and Items["Title"].Instance then
                        Items["Title"].Instance.Text = text
                    end
                    task.wait(Watermark.UpdateInterval)
                end
            end)
            TableInsert(Library.Threads, UpdateThread)
        end
        ...
        -- Change the format string at runtime.
        -- Supports tokens: {name} {username} {game} {gameid} {ping} {fps} {time} {date}
        function Watermark:SetFormat(fmt)
            Watermark.Format = fmt
        end

        -- Directly set a static text (no token replacement)
        ...
        -- Change update speed (seconds). Default = 1.
        function Watermark:SetUpdateInterval(interval)
            Watermark.UpdateInterval = interval
        end
    ...
    Library.Sections.Toggle = function(self, Data)
        ...
        local Items = { } do 
            ...
            Items["Toggle"]:OnHover(function()
                ...
                Items["Indicator"].Instance.BackgroundColor3 = Library.Theme["Hovered Element"]
    ...
    Library.Sections.Slider = function(self, Data)
        ...
        local Items = { } do 
            ...
            Items["RealSlider"]:OnHover(function()
                Items["RealSlider"].Instance.BackgroundColor3 = Library.Theme["Hovered Element"]
            ...
            Items["RealSlider"]:OnHoverLeave(function()
                Items["RealSlider"].Instance.BackgroundColor3 = Library.Theme["Background"]
    ...
    Library.Sections.Dropdown = function(self, Data)
        ...
        local Items = { } do
            ...
            local function UpdateDropdownHeight()
                ...
                local maxHeight = 90 -- max 6 items visible (~15px each)
                local newHeight = math.min(contentHeight, maxHeight)
                if newHeight < 1 then newHeight = 0 end
                Items["OptionHolder"].Instance.Size = UDim2New(1, 0, 0, newHeight)
            end

            Items["OptionHolder"].Instance:GetPropertyChangedSignal("AbsoluteCanvasSize"):Connect(UpdateDropdownHeight)
    ...
    Library.Sections.Textbox = function(self, Data)
        ...
        local Items = { } do 
            ...
            Items["Background"]:OnHover(function()
                Items["Background"].Instance.BackgroundColor3 = Library.Theme["Hovered Element"]
            ...
            Items["Background"]:OnHoverLeave(function()
                Items["Background"].Instance.BackgroundColor3 = Library.Theme["Element"]
